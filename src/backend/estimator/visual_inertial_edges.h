...
public:
    ...
    virtual void ComputeResidual() override {
        // Extract states.
        inv_depth0 = this->GetVertex(0)->param()(0);
        p_ic0 = this->GetVertex(1)->param();
        const TVec4<Scalar> &param_ic0 = this->GetVertex(2)->param();
        q_ic0 = TQuat<Scalar>(param_ic0(0), param_ic0(1), param_ic0(2), param_ic0(3));
        p_ic = this->GetVertex(3)->param();
    ...
    virtual void ComputeJacobians() override {
        ...
        const TQuat<Scalar> q_ci = q_ic.inverse();
        const TQuat<Scalar> q_cc0 = q_ci * q_ic0;
        const TMat3<Scalar> R_ci = q_ci.toRotationMatrix();
        const TMat3<Scalar> R_cc0 = q_cc0.toRotationMatrix();

        const TVec3<Scalar> jacobian_invdep = - R_cc0 * TVec3<Scalar>(norm_xy0.x(), norm_xy0.y(), static_cast<Scalar>(1)) / (inv_depth0 * inv_depth0);
